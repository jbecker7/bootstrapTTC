...
class Token:
    ...
    def checkIfKeyword(tokenText):
        for kind in TokenType:
            # This works b/c all of our enum vals for keywords are 1XX
...
class Lexer:
    ...
    # getToken gets called each time the compiler is ready for the next token
        ...
        if self.curChar == "+":
            ...
            if self.peek() == ".":
                ...
                if not self.peek().isdigit():
                    self.abort("Illegal character in number, weird")
                while self.peek().isdigit():
                    self.nextChar()
